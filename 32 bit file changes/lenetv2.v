...
module lenet(
	...
	// 28x28x6 -> depth 784 (rounded to 1024), word = 6 * `WDP = 192 bits
	reg		[9:0]	aa_conv1_buf;
	reg				cena_conv1_buf;
	reg		[9:0]	ab_conv1_buf;
	reg		[`WDP*`OUTPUT_NUM_CONV1-1:0]	db_conv1_buf;
	reg				cenb_conv1_buf;
	wire	[`WDP*`OUTPUT_NUM_CONV1-1:0]	qa_conv1_buf;
	...
	// 14x14x6 -> depth 196 (rounded to 256), word = 6 * `WDP = 192 bits
	reg		[7:0]	aa_relu1_buf;
	reg				cena_relu1_buf;
	reg		[7:0]	ab_relu1_buf;
	reg		[`WDP*`OUTPUT_NUM_CONV1-1:0]	db_relu1_buf;
	reg				cenb_relu1_buf;
	wire	[`WDP*`OUTPUT_NUM_CONV1-1:0]	qa_relu1_buf;
	...
	// Time-multiplexed: 4 parallel units run 4 passes to cover 16 channels.
	wire	[`WDP*`OUTPUT_NUM_CONV2_P -1:0] q_conv2;

	conv #(
		.INPUT_NUM		(`OUTPUT_NUM_CONV1),
		.OUTPUT_NUM		(`OUTPUT_NUM_CONV2_P),
		.WIGHT_SHIFT	(`WIGHT_SHIFT)
		)conv2(
		.clk			(clk),
		.rstn			(rstn),
	...
	localparam CONV2_POS_MAX = (`INPUT_WIDTH_CONV2 - `KERNEL_SIZE_CONV2 + 1)
	                         * (`INPUT_HEIGHT_CONV2 - `KERNEL_SIZE_CONV2 + 1) - 1;   // 99

	reg		[6:0]	aa_conv2_buf;
	reg				cena_conv2_buf;
	reg		[6:0]	ab_conv2_buf;
	reg		[1:0]	wbatch_conv2;
	reg		[`WDP*`OUTPUT_NUM_CONV2_P-1:0]	db_conv2_buf;
	reg				cenb_conv2_buf;
	wire	[0:`OUTPUT_BATCH_CONV2-1][`WDP*`OUTPUT_NUM_CONV2_P-1:0]	qa_conv2_bank;
	wire	[`WDP*`OUTPUT_NUM_CONV2-1:0]	qa_conv2_buf = qa_conv2_bank;

	genvar gb;
	generate
		for (gb = 0; gb < `OUTPUT_BATCH_CONV2; gb = gb + 1) begin : conv2_bank
			rfdp128x128 conv2_buf(
				.CLKA   (clk),
				.CENA   (cena_conv2_buf),
				.AA     (aa_conv2_buf),
				.QA     (qa_conv2_bank[gb]),
				.CLKB   (clk),
				.CENB   (cenb_conv2_buf | (wbatch_conv2 != gb[1:0])),
				.AB     (ab_conv2_buf),
				.DB     (db_conv2_buf)
				);
		end
	...
	// Position wraps every 100 writes; the wrap advances the destination bank.
	always @(`CLK_RST_EDGE)
		if (`RST)					ab_conv2_buf <= 0;
		else if (conv2_go)			ab_conv2_buf <= 0;
		else if (!cenb_conv2_buf)	ab_conv2_buf <= (ab_conv2_buf == CONV2_POS_MAX)
		                                            ? 7'd0 : ab_conv2_buf + 1'b1;
	always @(`CLK_RST_EDGE)
		if (`RST)					wbatch_conv2 <= 0;
		else if (conv2_go)			wbatch_conv2 <= 0;
		else if (!cenb_conv2_buf && ab_conv2_buf == CONV2_POS_MAX)
									wbatch_conv2 <= wbatch_conv2 + 1'b1;
	...
	reg		[`WDP*`OUTPUT_NUM_CONV2-1:0]	db_relu2_buf;
	reg				cenb_relu2_buf;
	wire	[`WDP*`OUTPUT_NUM_CONV2-1:0]	qa_relu2_buf;

	rfdp32x512 relu2_buf(
		.CLKA   (clk),
		.CENA   (cena_relu2_buf),
		.AA     (aa_relu2_buf),
		.QA     (qa_relu2_buf),
...
//	wire		[11:0]											aa_weight_FC1;
	...
	reg		[`WDP*`OUTPUT_NUM_FC1-1:0]	db_relu_fc1_buf;
	reg				cenb_relu_fc1_buf;
	wire	[`WDP*`OUTPUT_NUM_FC1-1:0]	qa_relu_fc1_buf;

	rfdp128x32 relu_fc1_buf(
		.CLKA   (clk),
...
//	wire		[11:0]											aa_weight_fc2;
	...
	reg		[`WDP*`OUTPUT_NUM_FC2-1:0]	db_relu_fc2_buf;
	reg				cenb_relu_fc2_buf;
	wire	[`WDP*`OUTPUT_NUM_FC2-1:0]	qa_relu_fc2_buf;

	rfdp128x32 relu_fc2_buf(
		.CLKA   (clk),
