...
generate 
	if (INPUT_NUM_PADING>1) begin : gen_sum_tree
		reg		[0:$clog2(INPUT_NUM_PADING)-1][0:(INPUT_NUM_PADING+1)/2 -1][`WDP*2-1:0]	sum = 0;
		for (i=0; i<$clog2(INPUT_NUM_PADING); i=i+1) begin : gen_level
			for(j=0; j < INPUT_NUM_PADING/(2**(i+1)); j++) begin : gen_j
				if (i==0) begin : gen_base
					always @(`CLK_RST_EDGE)
						if (`RST)	sum[i][j] <= 0;
			...
			end                                          // <-- closes gen_j (NEW)
		end                                              // closes gen_level
